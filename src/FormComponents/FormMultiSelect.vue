    ...
    <div class="form-group"
        ...
        >
            <template slot="tag" slot-scope="props">
                <span class="inline-block px-1 py-2 mr-2 mb-2 bg-grey-lighter rounded cursor-pointer">
                    <span>{{ props.option[fieldConfig.optionLabelField] }}</span>
                    <span class="text-base ml-1 font-black" @click="props.remove(props.option)">x</span>
                </span>
            </template>
        </multi-select>
...
export default {
    ...
    computed: {
        optionValue: {
            get() {
                if(!this.fieldConfig.options || this.fieldConfig.options.length === 0 || !this.value) {
                    return null;
                }
