...
export class FormErrors {
   ...
   * @param {boolean} first
   */
  get(field: string, first: boolean = true): string | string[] {

      if(this.has(field)) {
            let error = this.fieldErrors.filter({fieldName: field}).first() as ValidationErrorMessage;
            if(!error) {
                return '';
            }
  ...
  clear(field: string | null) {
    ...
    if (field) {
        let error = this.fieldErrors.filter({fieldName: field}).first();
        if(error) {
