<template>
  <div :id="'grid-' + uniqueId" class="grid">
    ...
    <div class="controls" v-show="!loadingData">
      <pagination
  ...
  setup(props, context: SetupContext) {
    ...
    const getRecords = async (page?: number) => {
      ...
      let data = await getRecordsFromAPI(
        getApiParamsForGrid(page, sortBy, currentFilter, recordUrlParams),
        apiClient.value,
        recordUrl.value,
        useJsonApi.value
      );
    ...
    let {
      ...
      from,
      to,
    } = setupPagination(gridData, pagination);
    ...
    let records = computed((): Model[] | Record<string, any>[] => {
      if (gridData.value instanceof Collection) {
        ...
        if (internalPagination.value && internalPage.value !== null) {
